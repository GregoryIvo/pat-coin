#!/usr/bin/env python3
# Copyright (c) 2021-2022 The Patriotcoin Core developers
...
class P2PPolicyTests(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # a Patriotcoin Core node that behaves similar to mainnet policies
    ...
    def run_test(self):
        ...
        ### test constants ###
        koinu = Decimal("0.00000001")          # 1 Koinu expressed in PATRIOT
        ten = Decimal("10.0")                  # uniform 10 PATRIOT seed moneys
        ...
        # create a bunch of UTXO with seed money from the Patriotcoin Core wallet
        for i in range(20):
        ...
        # test legacy output of 1 PATRIOT output and 1 PATRIOT fee
    ...
    # spend seed money with a key not in the Patriotcoin Core wallet.
    def spend_utxo(self, output, expected_size, retries=0):
        # construct the transaction using Patriotcoin Core raw tx APIs
        ...
        # and send the tx from there rather than from Patriotcoin Core, to test
