Patriotcoin Core version 1.14.7 is now available from:

  <https://github.com/patriotcoin/patriotcoin/releases/tag/v1.14.7/>

This is a new minor version release, including enchancements to several RPC
methods and important security updates for Patriotcoin-Qt. Patriotcoin-Qt users on
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/patriotcoin/lists/patriotcoin-releases>


Compatibility
==============

Patriotcoin Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
...
This release contains fixes for Patriotcoin Qt across all platforms that increase
security for end users.

### Disable BIP-70 payment server by default

To mitigate future potential risk inside Patriotcoin Qt, support for BIP-70 payment
...
inside Patriotcoin Qt. By default, incoming BIP-70 payment requests and files or
...
patriotcoin.conf or as an argument to Patriotcoin-Qt if absolutely needed. Please use
extreme caution when exercising this option.

This is step 1 in full deprecation of BIP-70, BIP-71 and BIP-72 inside Patriotcoin
...
* Updated the `rpcuser.py` script to improve the password and salt generation
  ...
  current authentication in `patriotcoin.conf` with new credentials generated with
...
Fee estimation was explicitly not ported or supported on Patriotcoin Core since
...
to support Patriotcoin parametrization.
...
```
patriotcoin-cli help getblockstats
```
...
* Fixed the example `patriotcoind.service` file (#3066)
* Fixed a bug where misnamed signal handlers were causing excessive warnings
  in Patriotcoin Qt logs (#3063)
...
* Updated the python `ltc-scrypt` module to a maintained fork (#3080), which
  can be found at https://github.com/patriotcoin/ltc-scrypt
