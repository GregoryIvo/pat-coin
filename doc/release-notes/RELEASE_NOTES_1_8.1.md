# Patriotcoin Core 1.8.1

Patriotcoin Core 1.8.1 is primarily a bugfix release, bringing Patriotcoin Core in
line with Bitcoin 0.9.3. Patriotcoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Patriotcoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Patriotcoin Core clients
* Use HTTPS with patriotchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/patriotcoin/patriotcoin/issues/134).
...
This only affects values significantly below 1 Patriot, however merchants may wish to round to
the nearest Patriot on all transactions for now.
