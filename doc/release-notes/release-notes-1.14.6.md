Patriotcoin Core version 1.14.6 is now available from:

  <https://github.com/patriotcoin/patriotcoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Patriotcoin Core users - miners, services,
...
Patriotcoin network from 1 PATRIOT to 0.01 PATRIOT. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 PATRIOT as
   change if possible.
2. If after signing there is more than 0.01 PATRIOT left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 PATRIOT, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 PATRIOT will not be accepted as valid by default.
...
```
patriotcoin-cli addnode <address> remove
```
...
```
patriotcoin-cli liststucktransactions
```
...
```
patriotcoin-cli setmaxconnections <desired_amount>
```
...
```
patriotcoin-cli rescan <start_block>
```
...
efforts to make Patriotcoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Patriotcoin Core.
