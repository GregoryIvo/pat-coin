...
Below are some notes on how to build Patriotcoin Core for Windows.
...
To build Patriotcoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Patriotcoin GitHub repository](https://github.com/patriotcoin/patriotcoin/)).
...
Note that for WSL v1 the Patriotcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/patriotcoin, AND not under, for example, /mnt/d/patriotcoin.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Patriotcoin's dependencies):
...
way. This will install to `c:\workspace\patriotcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/patriotcoin
