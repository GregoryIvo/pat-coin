...
const char * const BITCOIN_CONF_FILENAME = "patriotcoin.conf";
const char * const BITCOIN_PID_FILENAME = "patriotcoind.pid";
...
#else
    const char* pszModule = "patriotcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Patriotcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Patriotcoin";
#else
    // Unix
    return pathRet / ".patriotcoin";
#endif
#endif
}
