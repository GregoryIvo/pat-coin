...
#if defined(NDEBUG)
# error "Patriotcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Patriotcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetPatriotcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Patriotcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("patriotcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Patriotcoin: BIP30 has been active since inception
    ...
    // Patriotcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetPatriotcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Patriotcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Patriotcoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Patriotcoin: We probably want to disable this
    int nLockTimeFlags = 0;
